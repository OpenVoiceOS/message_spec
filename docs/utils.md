...
`IntentServiceInterface` now lives in `ovos-workshop` (`ovos_workshop.intents`). `IntentQueryApi` is
deprecated, as noted in its own section below. The bus message types and payloads are unchanged.
